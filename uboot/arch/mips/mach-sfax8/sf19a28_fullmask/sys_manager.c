...
*                                                                                                 *
***************************************************************************************************/
int sf_pad_set_value(u32 index)
{
	if(index > PAD_INDEX_MAX)
		return -1;

	writeb(1, (void *)GPIO_WDAT(0xB9D00000,index));
...
void sys_gmac_reset(void)
{
	...
	// the minimum value in MPW1 is 45 and 64 is a safe value.
	writew(64, (void *)EMAC_CLK_TX_I_DLY);
	writew(0x30, (void *)EMAC_CLK_PHY_RX_I_DLY);
...
void sys_sdio_reset(void)
{
	...
	//selete voltage 3.0V
	writew(0x00,(void*)(SW_MSC_REG));
}

/***************************************************************************************************
 *                                                                                                 *
 * -Description:                                                                                   *
 *    This part is used to reset gdu.                                                             *
 *                                                                                                 *
 ***************************************************************************************************/
void sys_module_select(enum bus_sel module)
{
	u32 tmp = readl((void*)REMAP_N);
	...
	writel(tmp, (void*)REMAP_N);
}

void sys_gdu_reset(void)
{
	sys_module_select(gdu);

	//reset emmc system manager module
	writew(0x0,(void*)BUS1_BM3_SYSM_RESET);
	writew(0x1,(void*)BUS1_BM3_SYSM_RESET);

	//enable gdu
	writew(0xFF,(void*)GDU_SOFT_CLK_EN);

	//hardward reset gdu
	writew(0xFF,(void*)GDU_SOFT_RESET);
	writew(0x00,(void*)GDU_SOFT_RESET);

	//set gdu boe
	writew(0x0F,(void*)GDU_SOFT_BOE);

}

void sys_crypto_reset(void)
{
	//reset crypto system manager module
	writew(0x0,(void*)BUS1_BM4_SYSM_RESET);
	writew(0x1,(void*)BUS1_BM4_SYSM_RESET);

	//enable crypto
	writew(0xFF,(void*)CRYPTO_SOFT_CLK_EN);

	//hardward reset crypto
	writew(0xFF,(void*)CRYPTO_SOFT_RESET);
	writew(0x00,(void*)CRYPTO_SOFT_RESET);

	//set crypto boe
	writew(0x0F,(void*)CRYPTO_SOFT_BOE);
...
void sys_remap_n_enable(void)
{
	...
	writel(readl((void *)REMAP_N) | 0x1,(void*)( REMAP_N));
}
...
void sys_reset_all_module(void)
{
	...
	sys_wifi_reset();
	sys_usb_reset();
#if defined(CONFIG_SFA18_RGMII_GMAC) || defined(CONFIG_SFA18_RMII_GMAC)
	sys_gmac_reset();
	printf("read gmac version 0x%x after reset",readl((void *)0xb0800020));
#else
	sys_gdu_reset();
#endif
	sys_emmc_reset();
