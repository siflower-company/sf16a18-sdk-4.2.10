...
int mtd_check_open(const char *mtd)
{
	...
	if(ioctl(fd, MEMGETOFFSET, &mtd_offset)) {
		fprintf(stderr, "Could not get MTD offset from %s\n", mtd);
		close(fd);
		return -1;
	}
...
resume:
	...
	w = e = 0;
	/* Align with erasesize to speed up erase speed. */
	rem = mtd_offset % erasesize;
	actual_erasesize = erasesize;
	for (;;) {
		if (rem)
			erasesize -= rem;
		...
		if (rem) {
			rem = (rem + buflen) % actual_erasesize;
			erasesize = actual_erasesize;
		}
