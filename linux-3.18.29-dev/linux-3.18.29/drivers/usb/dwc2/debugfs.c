...
static int usb_dbg_hotplug_test(struct seq_file *seq, void *v)
{
	struct dwc2_hsotg *hsotg = seq->private;

	printk("--USB hotplug test : enter test!\n");
...
static int usb_hotplug_open(struct inode *inode, struct file *file)
{
	return single_open(file, usb_dbg_hotplug_test, inode->i_private);
}

static const struct file_operations usb_dbg_hotplug_fops = {
	.owner		= THIS_MODULE,
	.open		= usb_hotplug_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static inline void dwc2_hsotg_create_hotplugtest_debug(struct dwc2_hsotg *hsotg) {
	struct dentry *root;

	root = hsotg->debug_root;

	if (!debugfs_create_file("usb_hotplug_test", S_IRUSR, root, hsotg,
					&usb_dbg_hotplug_fops))
			printk("--USB hotplug test : fail to test!\n");
}
...
int dwc2_debugfs_init(struct dwc2_hsotg *hsotg)
{
	...
	/*Add hotplug test debugfs node*/
	dwc2_hsotg_create_hotplugtest_debug(hsotg);
