...
static int xway_gphy_config_init(struct phy_device *phydev)
{
	...
	value = phy_read(phydev, XWAY_MII_CTRL);
	// pr_err("xway_gphy: read mii ctrl 0x%x\n", value);
	phy_write(phydev, XWAY_MII_CTRL, value|0x300 );
...
static int __init xway_phy_init(void)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(xway_gphy); i++) {
		int err = phy_driver_register(&xway_gphy[i]);
		if (err)
		  pr_err("xway_gphy: failed to load %s\n", xway_gphy[i].name);
	}
	return 0;
}

static void __exit xway_phy_exit(void)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(xway_gphy); i++)
	  phy_driver_unregister(&xway_gphy[i]);
}

module_init(xway_phy_init);
module_exit(xway_phy_exit);
