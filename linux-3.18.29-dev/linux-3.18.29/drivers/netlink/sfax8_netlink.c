...
int sf_genl_family_init(sf_nlfamily *sf_family, char *family_name, char *group_name){
	...
	printk("register generic netlink family name %s!\n",sf_family->family.name);
	...
	if (group_name){
		...
		ret = genl_register_family_with_ops_groups(&(sf_family->family), sf_genl_ops, sf_family->group);
	}else
	  ret = genl_register_family_with_ops(&(sf_family->family), sf_genl_ops);
