...
/*
 * Get offset of the partition. At which this region starts, from the beginning
 * of the flash.
 */
#define MEMGETOFFSET   _IOR('M', 25, uint64_t)
#endif
...
#ifdef CONFIG_SIFLOWER
       case MEMGETOFFSET:
               offset = mtdpart_get_offset(mtd);
               if (copy_to_user(argp, &offset, sizeof(uint64_t)))
                       return -EFAULT;
               break;
#endif
