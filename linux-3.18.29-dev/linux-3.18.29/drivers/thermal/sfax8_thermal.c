...
static int __sfax8_get_temperature(void *data, long *temp)
{
	...
	ret = sensor_data->get_temp(sensor_data->data, temp);
...
static int __sfax8_get_trend(void *data, long *trend)
{
	struct sfax8_sensor_data *sensor_data = data;
	long delta;
	...
	if (sensor_data->get_trend)
		return sensor_data->get_trend(sensor_data->data, trend);
				  ...
				  int (*get_trend)(void *, long *), void *data)
{
	...
	sdata->dev = dev;
	sdata->data = data;
	sdata->get_temp = get_temp;
	sdata->get_trend = get_trend;

	__get_trend = get_trend ? get_trend : __sfax8_get_trend;

	tzd = thermal_zone_of_sensor_register(dev, sensor_id, sdata,
		__sfax8_get_temperature, __sfax8_get_trend);
	if (IS_ERR(tzd))
		return PTR_ERR(tzd);
