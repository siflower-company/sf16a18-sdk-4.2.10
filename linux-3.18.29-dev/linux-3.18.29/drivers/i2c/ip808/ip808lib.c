...
const char *dev_ip808="/var/ip808";

#ifdef USER_API                           
int i2c_read_reg(unsigned char id, unsigned char reg_num, unsigned char *value)
{
	int fd,retval=-1;
	struct IP808General gs;
	memset(&gs,0x0,sizeof(struct IP808General));
	gs.i2caddr=(unsigned short)id;
	gs.regaddr=(unsigned short)reg_num;

	if((fd=open(dev_ip808,O_RDWR))<0)
	{	printf("Err: open %s failed!\n", dev_ip808); close(fd); goto read_out;}
	if(ioctl(fd,IP808_READ,&gs)<0)
	{  	printf("Err: ioctl %s failed!\n", dev_ip808); close(fd); goto read_out;}
	#ifdef USER_API_DEBUG
	printf("lib:ip808_read_reg addr=%02x data=%02x\n",gs.reg_addr,gs.reg_data);
	#endif   
	retval = close(fd);
	if(retval < 0)
	{	printf("Err: close %s failed!\n", fd); goto read_out;}
		
	*value = (unsigned char)gs.regdata;
	retval = 0;
	read_out:
		return retval;
}
int i2c_write_reg(unsigned char id, unsigned char reg_num, unsigned char value)
{
	...
	struct IP808General gs;
	#ifdef USER_API_DEBUG
	printf("lib:ip808_write_reg addr=%02x data=%02x\n",reg_num,value);
	#endif
	memset(&gs,0x0,sizeof(struct IP808General));
	gs.i2caddr=id;
	gs.regaddr=reg_num;
	gs.regdata=value;
	if((fd=open(dev_ip808,O_RDWR))<0)
	{  	printf("Err: open %s failed!\n", dev_ip808); close(fd); goto write_out;}
	if(ioctl(fd,IP808_WRITE,&gs)<0)
	{  	printf("Err: ioctl %s failed!\n", dev_ip808); close(fd); goto write_out;}
	retval = close(fd);
	if (retval < 0)
		printf("Err: close %s failed!\n", fd);   
...
#else
		printk("IP808 id[%x] init fail!\n",id);
#endif
	else
#ifdef USER_API
		printf("IP808 id[%x] init success!\n",id);
#else
		printk("IP808 id[%x] init success!\n",id);
#endif	
	return ret;
}
...
#ifndef USER_API
EXPORT_SYMBOL(IP808_init);
EXPORT_SYMBOL(i2c_write_reg);
EXPORT_SYMBOL(i2c_read_reg);
EXPORT_SYMBOL(get_page);
EXPORT_SYMBOL(set_page);
EXPORT_SYMBOL(get_device_address);
EXPORT_SYMBOL(get_op_mode);
EXPORT_SYMBOL(software_reset);
EXPORT_SYMBOL(get_led_enable);
EXPORT_SYMBOL(set_led_enable);
EXPORT_SYMBOL(get_led_order);
EXPORT_SYMBOL(set_led_order);
EXPORT_SYMBOL(get_led_act_lv);
EXPORT_SYMBOL(set_led_act_lv);
EXPORT_SYMBOL(get_led_init_lv);
EXPORT_SYMBOL(set_led_init_lv);
EXPORT_SYMBOL(get_led_clock_rate);
EXPORT_SYMBOL(set_led_clock_rate);
EXPORT_SYMBOL(get_led_mode);
EXPORT_SYMBOL(set_led_mode);
EXPORT_SYMBOL(get_led_start_idx);
EXPORT_SYMBOL(set_led_start_idx);
EXPORT_SYMBOL(get_power_mode);
EXPORT_SYMBOL(set_power_mode);
EXPORT_SYMBOL(get_trunk_power_limit);
EXPORT_SYMBOL(set_trunk_power_limit);
EXPORT_SYMBOL(get_trunk_select);
EXPORT_SYMBOL(set_trunk_select);
EXPORT_SYMBOL(get_port_event);
EXPORT_SYMBOL(clear_port_event);
EXPORT_SYMBOL(get_power_status);
EXPORT_SYMBOL(start_state_machine);
EXPORT_SYMBOL(get_port_state);
EXPORT_SYMBOL(set_port_state);
EXPORT_SYMBOL(get_af_at_mode);
EXPORT_SYMBOL(set_af_at_mode);
EXPORT_SYMBOL(get_af_at_mode_all);
EXPORT_SYMBOL(set_af_at_mode_all);
EXPORT_SYMBOL(get_dectect_sign);
EXPORT_SYMBOL(get_pd_class);
EXPORT_SYMBOL(get_port_current);
EXPORT_SYMBOL(get_port_temperature);
EXPORT_SYMBOL(get_supply_voltage);
EXPORT_SYMBOL(get_ivt_auto_poll);
EXPORT_SYMBOL(set_ivt_auto_poll);
EXPORT_SYMBOL(get_ivt_poll_period);
EXPORT_SYMBOL(set_ivt_poll_period);
