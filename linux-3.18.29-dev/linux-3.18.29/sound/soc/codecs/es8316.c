				...
				struct snd_soc_dai *dai)
{
	...
	u16 adciface = snd_soc_read(codec, ES8316_SDP_ADCFMT_REG0A) & 0xEF;
	u16 daciface = snd_soc_read(codec, ES8316_SDP_DACFMT_REG0B) & 0xEF;
	...
	/* set iface & srate*/
	snd_soc_update_bits(codec,ES8316_SDP_DACFMT_REG0B, 0xef, daciface);
	snd_soc_update_bits(codec, ES8316_SDP_ADCFMT_REG0A, 0xef, adciface);
...
static int es8316_init_regs(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, 0x22,0x30);  //CHARGE PUMP DIVIDER
    snd_soc_write(codec, 0x2F,0x00);
	...
	snd_soc_write(codec, 0x23,0x40);  //CLK ON
