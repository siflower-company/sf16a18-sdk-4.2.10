...
static int parse_force_name(char *start, char *mod_name, int mod_name_len)
{
    char *name_start;
    char *end = start;
    int force_name_len;
    ...
    if (!format_invalid) {
        printk("invalid force_mod_name type!\n");
        return -1;
    }
    ...
    force_name_len = end - name_start;
    if (force_name_len <= 0) {
        printk("too short force_mode_name length, %d!\n", force_name_len);
        return -2;
    }

    if (force_name_len >= mod_name_len) {
        printk("too long force_mod_name params, %d!\n", force_name_len);
        return -3;
    }
    strncpy(mod_name, name_start, force_name_len);
    mod_name[force_name_len] = '\0';
