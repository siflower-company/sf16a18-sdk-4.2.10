...
static inline int drv_agg_msdu(struct ieee80211_local *local,
			       struct amsdu_ctl *ctl, struct sk_buff *skb)
{
	return local->ops->agg_msdu(&local->hw, ctl, skb);
}
...
#ifdef CONFIG_MAC80211_PRIV_IOCTL
				   ...
				   u16 *ssn, u8 buf_size, bool *amsdu_supported)
{
	...
	if (local->ops->ampdu_action)
		...
		ret = local->ops->ampdu_action(&local->hw, &sdata->vif, action,
					       sta, tid, ssn, buf_size, amsdu_supported);
#else
		ret = local->ops->ampdu_action(&local->hw, &sdata->vif, action,
