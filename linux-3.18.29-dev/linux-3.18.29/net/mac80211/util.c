...
int ieee80211_amsdu_ctl(struct ieee80211_hw *hw, struct ieee80211_sta *pubsta,
			int tid, struct sk_buff *anchor, u8 next_pad, u16 left,
			bool rdy)
{
	...
	if (tid_tx && tid_tx->amsdu_on) {
		if (anchor) {
			if (tid_tx->amsdu_ctl.anchor != anchor) {
				tid_tx->amsdu_ctl.anchor = anchor;
				tid_tx->amsdu_ctl.next_pad = next_pad;
			}
			tid_tx->amsdu_ctl.left = left;
			tid_tx->amsdu_ctl.rdy = rdy;
			ret = 0;
		} else {
			tid_tx->amsdu_ctl.anchor = anchor;
			tid_tx->amsdu_ctl.rdy = rdy;
...
EXPORT_SYMBOL(ieee80211_amsdu_ctl);
...
#if defined(CONFIG_WIRELESS_VENDOR_SIFLOWER) || defined(CONFIG_WIRELESS_VENDOR_SIFLOWER_MODULE)
		ret = 0;
#else
